...
public class SubPlaneReimu extends BaseGameObject {
	...
	private void powerInc() {
		...
		int shooterInThis = 0;
		Shooter[] s = myPlane.shtFile.shootersList.get(myPlane.power);
		...
		s = myPlane.shtFile.shootersList.get(myPlane.power+ myPlane.shtFile.playerArg.maxPower+1);
		shooters.clear();
		shooterInThis = 0;
		for (Shooter sh : s) {
			if (sh.option == bianHao) {
		...
		case 3:
			...
			case 3:
				normalPos = myPlane.shtFile.subPlanePositions.get(p3n3);
				slowPos = myPlane.shtFile.subPlanePositions.get(p3s3);
				break;
			case 2:
				normalPos = myPlane.shtFile.subPlanePositions.get(p3n2);
				slowPos = myPlane.shtFile.subPlanePositions.get(p3s2);
				break;
			case 1:
				normalPos = myPlane.shtFile.subPlanePositions.get(p3n1);
				slowPos = myPlane.shtFile.subPlanePositions.get(p3s1);
				break;
			}
			break;
		case 2:
			switch (bianHao) { 
			case 2:
				normalPos = myPlane.shtFile.subPlanePositions.get(p2n2);
				slowPos = myPlane.shtFile.subPlanePositions.get(p2s2);
				break;
			case 1:
				normalPos = myPlane.shtFile.subPlanePositions.get(p2n1);
				slowPos = myPlane.shtFile.subPlanePositions.get(p2s1);
				break;
			} 
			case 1:
				normalPos = myPlane.shtFile.subPlanePositions.get(p1n);
				slowPos = myPlane.shtFile.subPlanePositions.get(p1s);
				break; 
		}
