...
import com.InsProcess.ChangeTask;
import com.InsProcess.ChangeTaskManager;
...
public class ShootLaser extends EclBullet {
	...
	public static void create(Enemy enemy, float centerX, float centerY, float length, float width,
		...
		ObjectPools.shootLaserPool.obtain().init(enemy, centerX, centerY, length, width, directionAngle, speed, voiceOnShoot,
				voiceOnChangeDirection, tasks);
	}

	public void init(Enemy enemy, float centerX, float centerY, float length, float width, float directionAngle,
			float speed, int voiceOnShoot, int voiceOnChangeDirection, ChangeTask[] tasks) {
		existTime = 0; 
	...
	@Override
	public void update() {
		System.out.println("update:" + objectCenter.x + " " + objectCenter.y);
		++existTime;
		...
		mid1.setPosition(begin1.getX(), begin1.getY());
		begin1.setRotation(degrees);
		mid1.setRotation(degrees);
		p1 = new Vector2(begin1.getX() + begin1.getOriginX(), begin1.getY() + begin1.getOriginY());
		p2 = new Vector2(
	...
	@Override
	public void judge() {
		if (pointToLine(p1.x, p1.y, p2.x, p2.y, MyPlane.instance.objectCenter.x,
				MyPlane.instance.objectCenter.y) < width) {
